...
p<-visualize_clusters(clu_res,tsne_proj[c("TSNE.1","TSNE.2")],title="kmeans clustering labels")
print(p)
dev.off()
for(n in n_clu){
...
p<-visualize_clusters(cluster_result$Cluster,tsne_proj[c("TSNE.1","TSNE.2")],title="Graph-based clustering labels")
print(p)
dev.off()
}
...
pdf(file=paste(outdir,filename,sep='/'))
p<-vis_clusters(mergerdsamplelabel,tsne_proj[c("TSNE.1","TSNE.2")],title="batch labels",colour=cols)
print(p)
dev.off()
}
